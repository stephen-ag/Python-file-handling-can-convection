...
def createTransientLoad(analysis):
    ...
    if analysis.AnalysisType.ToString() != "Transient" and analysis.PhysicsType.ToString() != "Thermal":
        # In case of other analysis/physics type.
        msg = "Selected Analysis is: " + analysis.Name + "\n"
        msg += "Load is applicable for Transient Thermal analysis only"
        ExtAPI.Application.LogWarning(msg)
        quit()
    ...
    for i in ids:
        ...
        Convection_vals =[]
        #cell values i+1 is selected because excel index starts at (1,1) where as python index is (0,0)
...
def createStructuralPressure(analysis):
    
    global Analysis_index, file_path, file_name, nstep_pressure
    """
       This method is called when the toolbar button "Transient Thermal Load" is clicked.
    ...
    conn1=model.Connections
        ...
        #-----------------------------------------------------------------
        # Check the Analysis and Physics type.
    if analysis.AnalysisType.ToString() != "Static Structural" and analysis.PhysicsType.ToString() != "Mechanical":
        # In case of other analysis/physics type.
        msg = "Selected Analysis is: " + analysis.Name + "\n"
        msg += "Load is applicable for Static Structural analysis with multiple Steps only"
        ExtAPI.Application.LogWarning(msg)
        quit()
    ...
    with open(timepath,"r") as r:
        ...
        datalist = data.split('\n')
    times = map(float,datalist)
    nstep_pressure = analysis.AnalysisSettings.NumberOfSteps
    ...
    excel_file_name = file_name
    workbook = myExcel.Workbooks.Open(os.path.join(mypath,excel_file_name))
    myWorkSheet = workbook.Worksheets[3]
    worsheetrange = myWorkSheet.UsedRange
    ...
    for i in ids:
        ...
        pressure.Name=str('Pressure_'+cv.Name)
        pressure.Magnitude.Inputs[0].DiscreteValues = time_vals

        #convection1.AmbientTemperature.Inputs[0].DiscreteValues = time_vals
        pressure_vals =[]
        #cell values i+1 is selected because excel index starts at (1,1) where as python index is (0,0)
        for p in range(2,rowcount+1):
            pressure_vals.append(Quantity("{} [MPa]".format(worsheetrange.Cells[p,i+1].Value2)))
        datas =  pressure_vals  
        pressure.Magnitude.Output.DiscreteValues = datas
...
def createStructuralForce(analysis):
    
    global Analysis_index, file_path, file_name, nstep_pressure, nstep_force
    """
       This method is called when the toolbar button "Transient Thermal Load" is clicked.
    ...
    nstep_force = analysis.AnalysisSettings.NumberOfSteps
    ...
    excel_file_name = file_name
    workbook = myExcel.Workbooks.Open(os.path.join(mypath,excel_file_name))
    myWorkSheet = workbook.Worksheets[4]
    worsheetrange = myWorkSheet.UsedRange
    ...
    for i in ids:
        ...
        cv=DataModel.GetObjectsByName(namedselection[i])[0]
        Force.Location= cv
        Force.Name=str('Force_'+cv.Name)
        Force.DefineBy = LoadDefineBy.Components
    
        Force.XComponent.Inputs[0].DiscreteValues = time_vals
        Force.YComponent.Inputs[0].DiscreteValues = time_vals
        Force.ZComponent.Inputs[0].DiscreteValues = time_vals        
        #convection1.AmbientTemperature.Inputs[0].DiscreteValues = time_vals
        ForceX =[]
        ForceY =[]
        ForceZ =[]
        ...
        #------------------------------X- Component -------------------------
        for p in range(3,rowcount+1):
            ForceX.append(Quantity("{} [N]".format(worsheetrange.Cells[p,i+1].Value2)))
    
        Force.XComponent.Output.DiscreteValues = ForceX 
        #------------------------------Y- Component -------------------------
        for q in range(3,rowcount+1):
            ForceY.append(Quantity("{} [N]".format(worsheetrange.Cells[q,i+2].Value2)))
          
        Force.YComponent.Output.DiscreteValues = ForceY
        
        #------------------------------Z- Component -------------------------
        for r in range(3,rowcount+1):
            ForceZ.append(Quantity("{} [N]".format(worsheetrange.Cells[r,i+3].Value2)))
    
        Force.ZComponent.Output.DiscreteValues = ForceZ
